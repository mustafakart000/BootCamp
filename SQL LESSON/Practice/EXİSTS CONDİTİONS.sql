...
INSERT INTO mart VALUES (20, '2Eddie', 'Toyota');
...
select musteri_isim, urun_isim from nisan where exists
(select musteri_isim from mart where nisan.urun_isim=mart.urun_isim );
                            
-------------------------------------------------------------------------------
--Her iki ayda ortak satilmayan ürünlerin URUN_ISIM'lerini ve bu ürünleri
--NİSAN ayında satın alan MUSTERI_ISIM'lerini listeleyen bir sorgu yazınız.

select urun_isim, musteri_isim from nisan where not exists(select musteri_isim from mart where nisan.urun_id = mart.urun_id);
select urun_isim, musteri_isim from mart where not exists(select musteri_isim from nisan where nisan.urun_id = mart.urun_id);